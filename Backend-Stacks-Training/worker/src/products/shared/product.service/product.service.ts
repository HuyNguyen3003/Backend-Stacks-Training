...
export class ProductApiBService {
  constructor(
    @InjectModel('ProductApiB')
    private readonly productApiBModel: Model<ProductApiB>,
  ) {}
  ...
  //   const find = await this.getAll(data.msg);
  //   if (find.length == 0) {
  ...
  async getAll(name: string): Promise<ProductApiB[]> {
    return this.productApiBModel.find({ Title: name }).exec();
  }
}
